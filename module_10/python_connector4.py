...
#    cursor = db.cursor()
    ...
    conn.commit()
    cursor.execute = ("CREATE USER 'outland_adventures_user'@'localhost' IDENTIFIED WITH       mysql_native_password BY 'MySQL8IsGreat!'")
    conn.commit()
    cursor.execute = ("GRANT ALL PRIVILEGES ON outland_adventures.*    TO'outland_adventures_user'@'localhost'")
    conn.commit()    
    cursor.execute = ("DROP TABLE IF EXISTS visa_tracker")
    conn.commit()
    
    sql = """CREATE TABLE visa_tracker (
